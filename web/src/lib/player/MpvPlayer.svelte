<script lang="ts">
  ...
  } from "$lib/player/progressSaver.svelte";
  ...
  const canPlay = $derived(mpvPlayer.ready && mpvPlayer.duration > 0);
  ...
  $effect(() => {
    if (!canPlay) return;
    progress.resume((t) => mpvPlayer.seek(t));
  });

  // Throttled save while playing (re-runs as position ticks).
  $effect(() => {
    const pos = mpvPlayer.position;
    if (!canPlay || !media || mpvPlayer.paused) return;
    progress.maybeSave(pos, mpvPlayer.duration, progressCtx);
  });

  // Mark complete at end of file.
  $effect(() => {
    if (mpvPlayer.ended && media)
      progress.saveNow(
        mpvPlayer.duration,
        mpvPlayer.duration,
        progressCtx,
  ...
  $effect(() => {
    if (appliedAudioDefault || mpvPlayer.audioTracks.length <= 1) return;
    const lang = $settings?.defaultAudioLang;
    if (!lang) return;
    appliedAudioDefault = true;
    const match = mpvPlayer.audioTracks.find((t) => t.lang === lang);
    if (match && !match.selected) mpvPlayer.setAudioTrack(match.id);
  });
  ...
  $effect(() => {
    ...
    const embedded = mpvPlayer.subtitleTracks.find((t) => t.lang === lang);
    if (embedded) {
  ...
  function toggleMute(): void {
    if (mpvPlayer.volume > 0) {
      lastVolume = mpvPlayer.volume;
      mpvPlayer.setVolume(0);
    } else {
      mpvPlayer.setVolume(lastVolume || 100);
    }
  }
  ...
  function onVolume(e: Event): void {
    mpvPlayer.setVolume(Number((e.target as HTMLInputElement).value));
  }
  ...
  function selectSubtitle(sel: SubSel): void {
    subSelection = sel;
    if (sel.kind === "off") {
      mpvPlayer.setSubtitleTrack(-1);
      return;
    }
    if (sel.kind === "embedded") {
      mpvPlayer.setSubtitleTrack(sel.id);
      return;
    }
    ...
    if (addedExternal.has(ext.id)) {
      ...
      const t = mpvPlayer.subtitleTracks.find((x) => x.lang === ext.lang);
      if (t) mpvPlayer.setSubtitleTrack(t.id);
    } else {
  ...
  const sortedAudio = $derived(
    [...mpvPlayer.audioTracks].sort((a, b) =>
  ...
  const subtitleGroups = $derived.by(() => {
    ...
    for (const t of mpvPlayer.subtitleTracks) {
  ...
  const selectedAudio = $derived(
    mpvPlayer.audioTracks.find((t) => t.selected),
  );
...
>
    ...
    >
      ...
      >
        ...
        <input
          ...
          max="100"
          step="1"
          value={mpvPlayer.volume}
          oninput={onVolume}
          class="h-1 w-20 cursor-pointer accent-white"
          aria-label="Volume"
        />

        <span class="text-sm tabular-nums">{fmt(mpvPlayer.position)}</span>
        <span class="text-sm text-white/50">/</span>
        <span class="text-sm text-white/70 tabular-nums"
          >{fmt(mpvPlayer.duration)}</span
        >

        <input
          type="range"
          min="0"
          max={mpvPlayer.duration || 0}
          step="0.1"
        ...
        <!-- Audio tracks -->
        {#if mpvPlayer.audioTracks.length > 1}
          <Popover.Root>
            <Popover.Trigger
              onclick={(e) => e.stopPropagation()}
            ...
            <Popover.Content side="top" class="w-52 p-1">
              {#each sortedAudio as track (track.id)}
                <button
                  onclick={() => mpvPlayer.setAudioTrack(track.id)}
        ...
        <!-- Subtitles -->
        {#if mpvPlayer.subtitleTracks.length > 0 || externalSubtitles.length > 0}
          <Popover.Root>
            <Popover.Trigger
              onclick={(e) => e.stopPropagation()}
              class="flex items-center gap-1.5 rounded-md px-2 py-1 text-white/70 transition hover:bg-white/10 hover:text-white"
  ...
  {#if mpvPlayer.available && compact}
  ...
  <!-- ── Loading screen ─────────────────────────────────────────────────────── -->
  {#if mpvPlayer.available && !canPlay && !compact}
